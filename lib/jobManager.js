...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName){
        ...
        var hashStateRoot = util.reverseBuffer(job.merkleTree.withFirst(coinbaseHash)).toString('hex');
        var hashUTXORoot = util.reverseBuffer(job.merkleTree.withFirst(coinbaseHash)).toString('hex');
        
        var headerBuffer = job.serializeHeader(merkleRoot, nTime, nonce, hashStateRoot, hashUTXORoot);
